...
cl run train-v1.1.json:"${train_file}" :dev-v1.1.json :docqa :glove :elmo-params nltk_data:nltk_data_docqa :eval_squad.py 'export PYTHONPATH=${PYTHONPATH}:`pwd`; export CUDA_VISIBLE_DEVICES='"${gpuid}"'; mkdir -p data/lm; cd data/lm; ln -s ../../elmo-params squad-context-concat-skip; cd -; python3 docqa/squad/build_squad_dataset.py --train_file train-v1.1.json --dev_file dev-v1.1.json; python3 docqa/elmo/ablate_elmo_model.py '"${mode} ${flags}"' model; python3 docqa/eval/squad_eval.py -o pred.json -c dev model*; python eval_squad.py dev-v1.1.json pred.json > eval.json'  --request-docker-image robinjia/tf-1.3.0-py3:1.0.1 -n "${name}" -d "${desc}" --request-queue host=${host}
